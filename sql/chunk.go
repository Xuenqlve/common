...
func NewChunkScanColumns(scanColumns []string) *ChunkRange {
...
func InitChunk(scanColumns []string, scanRange *Range) *ChunkRange {
	bounds := make([]*Bound, 0, len(scanColumns))
	columnOffset := map[string]int{}
	for index, column := range scanColumns {
		columnOffset[column] = index
...
func (r *ChunkRange) GetRange() *Range {
...
func (r *ChunkRange) ScanWhere(scanRange string) (where string, args []any) {
	where, args = r.toWhere(false)
	if where == "" {
		return scanRange, args
	} else if scanRange != "" {
		where = fmt.Sprintf("(%s) AND (%s)", where, scanRange)
	}
	where = RemoveRedundantParentheses(where)
	return
}

func (r *ChunkRange) ScanWhereNext(scanRange string) (where string, args []any) {
	where, args = r.toWhere(true)
	if where == "" {
		return scanRange, args
	} else if scanRange != "" {
		where = fmt.Sprintf("(%s) AND (%s)", where, scanRange)
...
func (r *ChunkRange) toWhere(next bool) (string, []any) {
	// 处理边界情况：没有Bounds
	if len(r.Bounds) == 0 {
		return "", nil
	}
	...
	// 单列情况下的简化处理
	if len(r.Bounds) == 1 {
		return r.whereSimpleColumn(next)
	}
	// 多列情况下处理
	return r.whereComplexColumn(next)
}

func (r *ChunkRange) whereSimpleColumn(next bool) (string, []any) {
	bound := r.Bounds[0]
	lowerSymbol := gte
	if next {
		lowerSymbol = gt
	...
	if bound.HasLower && !bound.HasUpper {
		return fmt.Sprintf("%s %s ?", ColumnName(bound.Column), lowerSymbol), []any{bound.Lower}
	}

	// 只有上界
	if !bound.HasLower && bound.HasUpper {
		return fmt.Sprintf("%s <= ?", ColumnName(bound.Column)), []any{bound.Upper}
	}

	// 同时有上下界且值相等
	if bound.HasLower && bound.HasUpper && bound.Lower == bound.Upper {
		return fmt.Sprintf("%s = ?", ColumnName(bound.Column)), []any{bound.Lower}
	}

	// 同时有上下界且值不等
	if bound.HasLower && bound.HasUpper {
		where := fmt.Sprintf("%s %s ? AND %s <= ?",
			ColumnName(bound.Column),
			lowerSymbol,
			ColumnName(bound.Column))
...
func (r *ChunkRange) whereComplexColumn(next bool) (string, []any) {
	// 为不同类型的条件初始化切片，预分配合理的容量避免频繁扩容
	sameCondition := make([]string, 0, len(r.Bounds))  // 存储值相等的条件（Lower==Upper）
	lowerCondition := make([]string, 0, len(r.Bounds)) // 存储下界条件
	upperCondition := make([]string, 0, len(r.Bounds)) // 存储上界条件

	sameArgs := make([]any, 0, len(r.Bounds))    // 相等条件的参数
	lowerArgs := make([]any, 0, len(r.Bounds)*2) // 下界条件的参数（可能包含前置条件参数）
	upperArgs := make([]any, 0, len(r.Bounds)*2) // 上界条件的参数（可能包含前置条件参数）

	// 用于构建复合条件的前置条件
	preConditionForLower := make([]string, 0, len(r.Bounds))
	preConditionForUpper := make([]string, 0, len(r.Bounds))
	preConditionArgsForLower := make([]any, 0, len(r.Bounds))
	preConditionArgsForUpper := make([]any, 0, len(r.Bounds))
	...
	for ; i < len(r.Bounds); i++ {
		...
		// 处理相等条件
		sameCondition = append(sameCondition, fmt.Sprintf("%s = ?", ColumnName(bound.Column)))
	...
	for ; i < len(r.Bounds); i++ {
		...
		if bound.HasLower {
			...
			// 更新前置条件，用于构建下一个列的条件
			preConditionForLower = append(preConditionForLower, fmt.Sprintf("%s = ?", ColumnName(bound.Column)))
		...
		if bound.HasUpper {
			...
			// 更新前置条件，用于构建下一个列的条件
			preConditionForUpper = append(preConditionForUpper, fmt.Sprintf("%s = ?", ColumnName(bound.Column)))
...
func (r *ChunkRange) Copy() *ChunkRange {
...
func (r *ChunkRange) UpdateLower(column string, lower any) {
...
func (r *ChunkRange) UpdateUpper(column string, upper any) {
	if offset, ok := r.columnOffset[column]; ok {
		// update the bound
...
func (r *ChunkRange) Update(column string, lower, upper any, updateLower, updateUpper bool) {
	if offset, ok := r.columnOffset[column]; ok {
		// update the bound
		if updateLower {
			r.Bounds[offset].Lower = lower
...
func (r *ChunkRange) Clone() *ChunkRange {
	newChunk := NewChunkRange()
	for _, bound := range r.Bounds {
		newChunk.addBound(&Bound{
			Column:   bound.Column,
...
func (r *ChunkRange) CopyAndUpdate(column string, lower, upper any, updateLower, updateUpper bool) *ChunkRange {
	newChunk := r.Copy()
	newChunk.Update(column, lower, upper, updateLower, updateUpper)
	return newChunk
}

func (r *ChunkRange) addBound(bound *Bound) {
